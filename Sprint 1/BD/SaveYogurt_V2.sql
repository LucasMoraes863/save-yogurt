...
CASE 
	WHEN temperatura < 1.0 THEN 'Alerta azul'
	WHEN temperatura >= 1.0 AND temperatura <= 4.0 THEN 'Temperatura ideal'
	WHEN temperatura > 4.0 AND temperatura < 10.0 THEN 'Alerta amarelo'
	WHEN temperatura >= 10.0 THEN 'Alerta vermelho'
END AS categoria_alerta,

-- Coluna do estado (Descrição da classificação)
CASE 
	WHEN temperatura < 1.0 THEN 'Estado crítico'
	WHEN temperatura >= 1.0 AND temperatura <= 4.0 THEN 'Temperatura ideal'
	WHEN temperatura > 4.0 AND temperatura < 10.0 THEN 'Estado intermediário'
